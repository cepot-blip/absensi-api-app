...
/**
 * @function AbsenMasukRead ini digunakan untuk menampilkan data absen yang sudah dibuat
...
export const AbsenMasukRead = async (req = request, res =  response) => {
    try {
        const page = parseInt(req.query.page) || 1;
        const limit = parseInt(req.query.limit) || 10;
        const skip = (page - 1) * limit;
        const filter = req.body.filter ?? {};
        const result = await AbsenMasukModels.findMany({
          skip: skip,
          take: limit,
          orderBy: { id: 'desc' },
          where: filter,
        ...
        const conn = await AbsenMasukModels.count();
